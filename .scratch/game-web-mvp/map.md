...
- [12 - 黑市牌效果注册](issues/12-黑市牌效果注册.md)：52 张黑市牌注册完成（24 黄边全量 + 非黄边尽量）。强化芯片工厂 chipInstall（金科玉律 3/4：每牌限 1、点数 2-14 校验、无目标弃置，数值类 resolve 用 addPermanentRank）；血幕镀层（出/夺）以 (duel,during) 注册对持有者实际生效；秘密交易立即结算（027/031/034/036/039 + 闭店礼/血筹分享/拔除芯片）；道具统一注册 run 存 items（规避 whiteboard"备用道具"与 JSON"道具"命名不一致）。依赖【临时特权证】状态/黑市区 slot 交互/hand-evaluator 芯片视图的声明效果占位 TODO，遗留已记录。
